...
#bucket = s3.Bucket('ai-sim-cloud-639962993200')
dynamodb = boto3.resource('dynamodb')
table = dynamodb.Table("ai-sim-cloud_data")
...
def display_image(image):
  ...
  plt.grid(False)
  # plt.imshow(image)
  plt.imsave("/var/task/result.jpg", image)

def download_and_resize_image(url, new_width=256, new_height=256,
                              display=False):
  global exec_id, send_data
  filename = None
...
def run_detector(detector, path):
  global exec_id, send_data, src_img
...
def send(send_data):
    ...
    try:
        response = requests.post(url, json=json_message, timeout=5)
        if not response.ok:
...
def server_run(data):
  ...
  decoded_b = base64.b64decode(data.encode())
...
def save_time_data():
  ...
  el_time = np.array(end_time) - np.array(start_time)

  data = {"Exec_id": log_time.isoformat(),
    "st1": start_time[0], "st2": start_time[1], "st3": start_time[2], "st4": start_time[3], "st5": start_time[4],
    "st6": start_time[5], "st7": start_time[6], "st8": start_time[7], "st9": start_time[8],
    "et1": end_time[0], "et2": end_time[1], "et3": end_time[2], "et4": end_time[3], "et5": end_time[4],
    "et6": end_time[5], "et7": end_time[6], "et8": end_time[7], "et9": end_time[8],
    "ell": el_time[0], "el2": el_time[1], "el3": el_time[2], "el4": el_time[3], "el5": el_time[4], 
    "el6": el_time[5], "el7": el_time[6], "el8": el_time[7], "el9": el_time[8] 
  }
  item = json.loads(json.dumps(data), parse_float=decimal.Decimal)
  table.put_item(Item = item)

def handler(event, context):
  server_run(event["data"])
